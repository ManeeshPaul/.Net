...
namespace Part2_CreatePdfFromCSV
{
                ...
                foreach (var record in records)
                {
                    foreach (var field in record)
                    {
                        table.AddCell(new Cell()
                                ...
                                .SetTextAlignment(TextAlignment.CENTER)); // Add each field from the record
                    }
                }
        ...
        private static List<string[]> ReadCsvData(string path, out string[] headers)
        {
            ...
            using (var csv = new CsvReader(reader, System.Globalization.CultureInfo.InvariantCulture))
            {
                ...
                while (csv.Read())
                {
                    ...
                    var record = new string[headers.Length]; // Use the length of headers
                    for (int i = 0; i < headers.Length; i++)
                    {
                        record[i] = csv.GetField(i); // Get field by index
                    }
                    records.Add(record);
                }
            }
